...
def get_discriminator(latent_extractor_ckpt, discriminator_ckpt, enable_grad=True):
    classifier = latent_extractor_ckpt
    discriminator = discriminator_ckpt
...
def get_reverse_discriminator(latent_extractor_ckpt, discriminator_ckpt, enable_grad=True):
    classifier = latent_extractor_ckpt
    discriminator = discriminator_ckpt
    def evaluate(perturbed_inputs, timesteps=None, condition=None):
        with torch.enable_grad() if enable_grad else torch.no_grad():
...
def load_attribute_classifier(ckpt_path, device, condition, eval=False, channel=512):
    discriminator_args = dict(
      image_size=8,
      classifier_use_fp16=False,
      classifier_width=128,
...
def get_weight_correction(discriminator, vpsde, unnormalized_input, std_wve_t, img_resolution, time_min, time_max, class_labels=None):
    ...
    if tau.min() > time_max or tau.min() < time_min or discriminator == None:
      return torch.zeros_like(unnormalized_input), 10000000. * torch.ones(unnormalized_input.shape[0], device=unnormalized_input.device), torch.zeros_like(unnormalized_input)
    ...
    with torch.enable_grad():
        ...
        log_ratio, prediction = get_log_ratio(discriminator, x_, tau, class_labels)
        correction = torch.autograd.grad(outputs=prediction.sum(), inputs=x_, retain_graph=False)[0]
        correction *= - ((std_wve_t[:,None,None,None] ** 2) * mean_vp_tau[:,None,None,None]) ## Scaling to Expected Denoised Point
...
def get_grad_log_ratio(discriminator, vpsde, unnormalized_input, std_wve_t, img_resolution, time_min, time_max, class_labels, log=False):
    ...
    if log:
        return discriminator_guidance_score, log_ratio, prediction
    return discriminator_guidance_score

def get_prediction(discriminator, vpsde, unnormalized_input, std_wve_t, img_resolution, time_min, time_max, class_labels, log=False):
    mean_vp_tau, tau = vpsde.transform_unnormalized_wve_to_normalized_vp(std_wve_t) ## VP pretrained classifier
    if tau.min() > time_max or tau.min() < time_min or discriminator == None:
        if log:
          return torch.zeros_like(unnormalized_input), 10000000. * torch.ones(unnormalized_input.shape[0], device=unnormalized_input.device), torch.zeros_like(unnormalized_input)
...
class vpsde():
    ...
    def T(self):
        return 1
    def compute_reverse_tau(self,tau):
        tau *= self.beta_1 - self.beta_0
        std_wve_t = (tau+self.beta_0) **2 - (self.beta_0 ** 2.)
        std_wve_t /= 2. * (self.beta_1 - self.beta_0)
        std_wve_t = torch.exp(std_wve_t) -1
        std_wve_t = std_wve_t **0.5
...
# for i in range(100):
#     mean_vp_tau, tau = a.transform_unnormalized_wve_to_normalized_vp(torch.tensor(80.))
#     print(i/100, tau)

# compute_reverse_tau

q = a.compute_reverse_tau(torch.tensor(0.))
print(q)